(() => {
  ...
  function renderMatches() {
    ...
    if (!state.matches.length) {
      body.innerHTML = `<tr><td class="rotation-sheet-empty" colspan="${courtCount + 2}">No matches yet. Generate them from Session.</td></tr>`;
      return;
    }
    ...
    for (const [round, matches] of grouped.entries()) {
      ...
      rows.push(`
        <tr class="${allCompleted ? 'completed-rotation' : ''}">
          ...
          <td class="rotation-meta-cell">${fmtTime(first.start)}–${fmtTime(first.end)}</td>
          ${courtCells}
        </tr>
      `);
    }
    ...
    body.innerHTML = rows.length
      ? rows.join('')
      : `<tr><td class="rotation-sheet-empty" colspan="${courtCount + 2}">All matches are completed.</td></tr>`;
